...
export default function Home() {
  ...
  const getMovies = async () => {
    const movies = await axios.get(
      `https://api.themoviedb.org/3/${
        searchValue === "" ? "discover" : "search"
      }/movie?language=en&with_genres=${genreId}&query=${searchValue}`,
      {
        headers: {
  ...
  const getUpComingMovies = async () => {
    const upComingMovies = await instance.get(
      `/movie/upcoming?language=en&with_genres=${genreId}&query=${searchValue}`
    );
    setUpComingMovies(upComingMovies.data.results);
  };
  const getTopRatedMovies = async () => {
    const topRatedMovies = await instance.get(
      `/movie/top_rated?language=en&with_genres=${genreId}&query=${searchValue}`
    );
    setTopRatedMovies(topRatedMovies.data.results);
  };
  const getPopularMovies = async () => {
    const popularMovies = await instance.get(
      `/movie/popular?language=en&with_genres=${genreId}&query=${searchValue}`
    );
  ...
  const movieRanks = ["Popular", "Top rated", "Upcoming"];
  return (
    <div className="p-0">
      ...
      {movieRanks.map((rank, index) => {
        return (
          <div key={index} className="w-[1438px] mx-auto my-12">
